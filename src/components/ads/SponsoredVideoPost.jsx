...
export default function SponsoredVideoPost({ ad, onImpression, onClick }) {
  ...
  const posterUrl = ad?.content?.posterUrl
  ...
  if (!ad || !videoUrl) return null

  return (
    <section
      className="rounded-lg border-2 border-linkedin-primary/15 bg-linkedin-white/95 p-0 shadow-sm overflow-hidden"
      data-sponsored-video
    >
      {/* Promoted badge + Why am I seeing this */}
      <div className="flex items-center justify-between px-4 pt-3">
        <span className="inline-flex items-center gap-1 text-xs font-medium uppercase tracking-wide text-linkedin-text-gray">
          <Play className="h-3 w-3" /> Promoted video
        </span>
        <button
          type="button"
